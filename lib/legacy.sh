...
legacy::removed_handler() {
	...
	output::error <<-EOF
		...
		The buildpack you're using is likely employing Scalingo's jvm-common
		buildpack to install OpenJDK.

		Your buildpack is using the bash function ${function_name} which is no
		longer exposed in the latest release of Scalingo's jvm-common buildpack.
		If you didn't upgrade anything, it's likely that your buildpack is
		implicitly using the latest release of Scalingo's jvm-common buildpack.
